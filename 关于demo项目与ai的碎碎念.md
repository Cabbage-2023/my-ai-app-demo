...
# 心得体会 2 — 前端 AI 项目到底有哪些种类

## 四大类

### 1. AI Native 应用（我在走的路）

用 LLM API 搭面向用户的 AI 功能。**核心：AI 是产品的核心体验。**

| 子类 | 例子 | 技术点 |
|------|------|--------|
| AI Chat / 助手 | 各种 ChatBot | Vercel AI SDK，流式渲染 |
| RAG 问答 | 知识库问答、文档助手 | 向量检索、chunking、reranker |
| AI Agent | 自动执行任务的助手 | Tool Calling、多步推理 |
| AI 内容生成 | AI 写作、PPT 生成 | Prompt 工程、结构化输出 |

这是最主流、岗位最多的方向。

### 2. AI 工具链 / 基础设施

给 AI 开发者用的工具。**核心：让 AI 开发更爽。**

| 子类 | 例子 | 技术点 |
|------|------|--------|
| Prompt 管理 | Prompt 调试器、版本管理 | 编辑器、可视化 |
| AI 调试工具 | LLM 调用链路追踪 | 数据流监控 |
| 模型托管 UI | Hugging Face 上的 demo 页面 | 模型推理 + 前端展示 |

岗位相对少，但含金量高。

### 3. 传统前端 + AI 增强

AI 是配角，用来优化已有产品功能。**核心：AI 让传统功能更好用。**

| 子类 | 例子 | 技术点 |
|------|------|--------|
| AI 搜索 | 电商搜索、内容搜索 | RAG + 排序 |
| AI 推荐 | 个性化内容推荐 | 向量召回、用户画像 |
| AI 辅助输入 | 智能表单、自动补全 | 小模型推理、结构化输出 |
| AI 审核 | 内容审核、评论过滤 | 分类模型、规则引擎 |

**这个才是前端岗里最多的 AI 场景**——不是在做一个 AI 产品，而是在传统产品里用 AI 解决一个具体问题。

### 4. AI 绘画 / 多模态

图像、音频相关的 AI 前端。**核心：处理非文本数据。**

| 子类 | 例子 | 技术点 |
|------|------|--------|
| AI 绘图 UI | Stable Diffusion WebUI | ComfyUI 工作流、Canvas |
| AI 视频/音频 | 语音合成、数字人 | WebSocket 流式处理 |
| 多模态展示 | 图片理解、OCR 结果展示 | 大文件上传、Canvas 渲染 |

## 所以我在哪

我在 **1（AI Native）+ 3（RAG 增强）** 的交集上。中流大厂的前端 AI 岗，技术栈大概率就是这三样：

```
LLM API（DeepSeek/GLM/Qwen） + 向量库（MongoDB/Pinecone） + AI SDK（Vercel AI/LangChain）
```

拼不同的业务场景：

- 电商 → 商品知识库 RAG（第 3 类）
- 客服 → 工单系统 + AI 辅助回答（第 3 类）
- 内容平台 → 创作助手 / 智能推荐（第 3 类）
- 内部工具 → 文档助手 / 代码助手（第 1 类）

RAG 这个方向是最通用、岗位最多、面试最好聊的方向。

## 真正"走歪了"的信号

```
❌ 做个 AI 聊天框，说这是 AI 项目       →  面试官：这不就调了个 API？
❌ 用 LangChain 搭了一堆链，实际不懂     →  面试官：你换一个框架还能做吗？
❌ 光做 RAG 不做评估                     →  面试官：你怎么知道它好用？
```

## 一个真实的竞争力问题

前端 AI 岗面到最后，面试官关心的就两个问题：

1. 你能不能把 AI 功能落地到产品里？→ RAG 项目回答了这个问题
2. 你遇到问题了能不能自己排查？→ 评估脚本回答了这个问题

如果觉得"奇怪"，是因为一直在跟 AI 算法岗的人比较。但对标不是他们——对标是其他前端。大部分前端连 RAG 是什么都说不清楚，你已经能讲 chunking 策略和 reranker 对比了。这就够了。
...
基础 RAG 用的是 MongoDB Atlas $vectorSearch，但 Atlas M0 免费版限制 512MB 存储。迁移到腾讯云 4C4G 服务器，MongoDB Community 退化为备份存储，Qdrant（Docker）承担向量搜索。

当前状态：
- Qdrant 28,602 条向量，MongoDB 24,845 条文档
- Qdrant payload 直接存 content + metadata，搜索即用
- 新增 `scripts/lib/qdrant.ts`、`scripts/load-to-qdrant.ts`、`src/lib/qdrant.ts`

## 阶段 2：RAG 检索质量评估脚本（第二版）

### 设计
82 条 QA 对（15 事实 + 51 观点 + 16 对比），全部基于 Qdrant 真实数据。新增 Precision@K、Type Accuracy、Source Diversity 指标。

### 基线结果（纯向量搜索）

**命中率：**

| 分类 | Hit@1 (模) | Hit@3 (模) | MRR (模) |
|------|:---:|:---:|:---:|
...
1. **Type Accuracy 极低（0.5%）**— 搜游戏简介时 top-5 大多是评论而非 `game_intro`。Phase 2 type filter 预期从 2.7% → 90%+。
2. **对比类严格 Hit@3=43.8%**— 跨游戏对比跨半数 expect 匹配失败，模糊 81.3% 说明语义相关但命名不一致。
3. **4 题完全未命中**：Saber（英文名无数据）、素晴日掉书袋（语义鸿沟）、评价音乐画面（无锚点）、柚子社推荐（别名不匹配）。纯向量搜索的固有边界。
4. **Prec@5 ≈ 55%**— top-5 中约一半与主题相关。Reranker（Phase 3）目标是提到 80%+。

### 数据探查脚本

新增 `scripts/query/` 目录，包含四个数据探查脚本，用于在编写 QA 前查看 Qdrant 真实数据分布：
- `list-games-chars.ts` / `list-chars.ts` — 列出游戏名和角色名
- `sample-content.ts` / `sample-comparison.ts` — 各类型内容样本
